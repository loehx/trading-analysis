...
const SOURCE_FILES = '/Users/alx/Projekte/temp/output/[PAIR]/*.zip';
...
(async () => {
	try {
		...
		for (const pair of PAIRS) {
			const searchpath = SOURCE_FILES.replace('[PAIR]', pair.toLowerCase());
			log.write(`start converting data: "${pair}" => ${searchpath}`)
			const zipFiles = await glob.promise(SOURCE_FILES);
			...
			log.writeProgress(counter += 1, zipFiles.length);
			return;
			log.stopTimer('done');
		}
	}
